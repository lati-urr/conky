...
function conky_main()
    ...
    if text_elements ~= nil then
      for i in pairs(text_elements) do
        write_text(display, text_elements[i])
      end
    end

    cairo_select_font_face(display, 'Font Awesome', CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL)
    cairo_set_font_size(display, 36)

    local te = cairo_text_extents_t:create()
    local fe = cairo_font_extents_t:create()
    local handle = io.popen("upower -i $(upower -e | grep BAT1) | grep state | awk '{print $2}'")
    local battery_state = handle:read("*a")
    handle:close()
    local battery_state = string.gsub(battery_state, "\n", "")
    if battery_state == 'discharging' then
      text = ''
    elseif battery_state == 'charging' then
      text = ''
    elseif battery_state == 'fully-charged' then
      cairo_set_source_rgba(display, hexa_to_rgb( 0x00e5ff, 1))
      text = ''
    else
      text = '？'
    end
    cairo_text_extents(display, text, te)
    cairo_font_extents(display, fe)
    x = 160 - te.width/2 - te.x_bearing
    y = 200 + te.height/2
...
function write_text(cr, def)
  ...
  cairo_set_source_rgba(cr, hexa_to_rgb( def.color or 0x127ee1, def.alpha or 1))
  local text
  if def.sh ~= nil then
    local handle = io.popen(def.sh)
    local result = handle:read("*a")
    handle:close()
    text = string.gsub(result, "\n", "")
  else

...
function conky_main()
      ...
      if (graph_settings[i].draw_me==nil or conky_parse(tostring(graph_settings[i].draw_me)) == "1") then
        ...
        for j =1, nb_values do
          ...
          if j==nb_values then
            ...
            if value >= graph_settings[i].max and not graph_settings[i].autoscale then
              graph_settings[i].values[nb_values]=graph_settings[i].max
            else
  ...
  if text_settings ~= nil then
    for i in pairs(text_settings) do
      write_text(cr, text_settings[i], updates)
    end
    conky_start = nil
  end
